...
def myplot(p,iter,save = False, name = "",col = 'black'):
    ...
    plt.title("it = %d"%iter)
    if save: 
        plt.savefig('../output_images/%s_plot_%03d.png'%(name,iter))
        plt.clf()
    else:
        plt.show() 
    

def GradDescent(J,gradJ,pini,pi,xgrid,ygrid,itermax,learning_rate =0.1, test_number = 1): 
    p = np.copy(pini)
    # y = [p]
    eps = 1e-10
    err = 2*eps
    ...
    while err>eps and iter<itermax:
        ...
        iter += 1
        if iter%10==0:
            myplot(p,iter,True,"withoutWolfe")
        t = time.process_time() - t00
        file1 = open("GradDescent_100.txt","a")#append mode
        file1.write("Iter: %03d with Erreur: %.6f, running time %.3f\n"%(iter,err,t))
        file1.close()
    return p, iter 
...
def GradDescent_wolfe(J,gradJ,pini,pi,xgrid,ygrid,itermax,step =0.1): 
    ...
    while err>eps and iter<itermax:
        ...
        iter += 1
        if iter%5==0:
            myplot(p,iter, True, "wolfe_100_")
        t1 = time.process_time() - t00
        file1 = open("GradDescent_wolfe_100.txt","a")#append mode
        file1.write("Iter: %03d Erreur: %.7f runningtime %.3f \n"%(iter,err,t1))
        file1.close()
        print(iter)
    return p, iter 

def BFGS(f,gradf,x0, B0, eps = 1e-8, itermax = 100):
#     H0 is random symetric definite positive matrix
    ...
    while iter < itermax : 
        ...
        if np.norm(grad) < eps:
            break 
        d = -B.dot(gradf(x))
        step = Wolfe_learning_rate(f, gradf, x, d)
        x = x + step*B.dot(gradf(x))
        s = step*d 
        y = gradf(x).ravel() - grad 
        M = I - (s.dot(y.T))/(y.T.dot(s))
        B = M.T.dot(B).dot(M) + (s.dot(s.T))/(y.T.dot(y))
    return x 
